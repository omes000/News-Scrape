...
function savedArticles () {
	$.get("/api/saved").then((data) => {
		console.log(data);
		for(var i = 0; i < data.length; i++){
			$("#saved-articles").append("<div class='panel panel-default' data-id='"+ data[i]._id +"'><div class='panel-heading'><h3 class='panel-title'>" + data[i].title + "</h3><button type = 'button' class = 'btn btn-default pull-right remove-article' id ='" + data[i]._id +"'>Remove Article</button><button type = 'button' class = 'btn btn-default pull-right comment-article' id ='" + data[i]._id +"'>Comment</button></div><div class='panel-body'><a href ='" + data[i].link + "'>"+ data[i].link +"</a><div id = 'notes'></div></div></div>")
		}
	})
}
